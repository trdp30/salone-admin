import {
  CATEGORIES_REQUEST_INITIATED,
  CATEGORIES_REQUEST_SUCCEED,
...
export const createCategory = (data) => {
  return function (dispatch) {
    if (data && Object.keys(data) && !Object.keys(data).length) {
      return dispatch(
        catchReduxError(CATEGORIES_REQUEST_FAILED, "'data' cannot be empty")
      );
    }
    dispatch(actionInitiated(CATEGORIES_REQUEST_INITIATED));
...
export const updateCategory = (id, data) => {
  return function (dispatch) {
    if (id) {
      return dispatch(
        catchReduxError(CATEGORIES_REQUEST_FAILED, "'id' cannot be empty")
      );
    }
    if (data && Object.keys(data) && !Object.keys(data).length) {
      return dispatch(
