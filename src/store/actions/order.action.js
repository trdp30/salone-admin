import {
  ORDERS_REQUEST_INITIATED,
  ORDERS_REQUEST_SUCCEED,
    ...
    return query("order", {
      ...
      .then((response) => {
        if (response && response.data && response.data.length) {
          dispatch(
            fetchUsers({
              id: uniq(response.data.map((res) => res.user_id)),
            })
          );
        ...
        dispatch(
          normalizedData({
            ...
            relationShips: [
              {
                modelName: "cartItems",
                actionType: CARTITEMS_REQUEST_SUCCEED,
              },
              {
                modelName: "appointment",
                actionType: APPOINTMENT_REQUEST_SUCCEED,
              },
              {
                modelName: "items",
                actionType: ITEMS_REQUEST_SUCCEED,
              },
              {
                modelName: "packages",
                actionType: PACKAGES_REQUEST_SUCCEED,
              },
            ],
          })
        )
...
export const createOrder = (data) => {
  return function (dispatch) {
    if (data && Object.keys(data) && !Object.keys(data).length) {
      return dispatch(
        catchReduxError(ORDERS_REQUEST_FAILED, "'data' cannot be empty")
      );
    }
    dispatch(actionInitiated(ORDERS_REQUEST_INITIATED));
...
export const updateOrder = (id, data) => {
  return function (dispatch) {
    if (id) {
      return dispatch(
        catchReduxError(ORDERS_REQUEST_FAILED, "'id' cannot be empty")
      );
    }
    if (data && Object.keys(data) && !Object.keys(data).length) {
      return dispatch(
