...
export const addItem = (key, value, type = "object") => {
  return async function (dispatch, getState) {
    ...
    if (type == "array" && key == "questionStat" && appState.data[key]) {
      let index = findIndex(appState.data[key], [
        "question_id",
        parseInt(value.question_id),
      ]);
      if (index != -1) {
        appState.data[key] = appState.data[key].filter(
          (data) => data.question_id !== value.question_id
        );
      }
